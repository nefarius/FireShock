...
If a DualShock 3 gets connected to the USB hub, the filter will send a "magic" start packet to the control endpoint so the controller will continously start sending HID input reports via the interrupt in endpoint on interface 0.
